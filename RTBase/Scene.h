...
public:
	...
	Ray generateRay(float x, float y)
	{
		...
		Vec3 clipPoint(ndcX, ndcY, 0.0f, 1.0f);
		Vec3 cameraPoint = inverseProjectionMatrix.mulPoint(clipPoint);
		cameraPoint = cameraPoint.perspectiveDivide();
		Vec3 worldPoint = camera.mulPoint(cameraPoint);
		Vec3 dir = (worldPoint - origin).normalize();
